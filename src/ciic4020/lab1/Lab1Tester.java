...
public class Lab1Tester {
    ...
    public void testMoreFrequent() {
        ...
        Assert.assertTrue("Resulting bag must be empty", b_1.isEmpty());
        b_1 = bag_s.moreFrequentThan("");
        Assert.assertTrue("Resulting bag must have all the individual names", b_1.isMember("James") &&
                b_1.isMember("Rick") &&
                b_1.isMember("Dio") &&
                b_1.isMember("Carl"));
        b_1 = bag_s.moreFrequentThan("Dio");
        Assert.assertTrue("Resulting bag must only contain James", b_1.size() == 1 && b_1.isMember("James"));
    }
    ...
    public void testEquals() {
        ...
        Assert.assertFalse("Sets are not equal", s_1.equals(s_2));
        Assert.assertTrue("Sets are equal", s_1.equals(s_4));
        Assert.assertTrue("Sets are equal", s_2.equals(s_3));
    ...
    public void testDisjoint() {
        ...
        Assert.assertTrue("Currently there are no disjointed sets", check == false);
        sets[3] = s_4;
        check = DynamicSet.checkDisjoint(sets);
        Assert.assertTrue("Currently there are disjointed sets", check == true);
    }
    ...
    public void testSingleton() {
        ...
        Assert.assertTrue("Should have same size",  singl_1.size() == s_1.size());
        for (Set<String> set : singl_1) {
            Assert.assertTrue("Singleton should have only 1 element", set.size() == 1);
            for (String subElement : set) {
                Assert.assertTrue("Singleton element not in original set", s_1.isMember(subElement));
                s_1.remove(subElement);
            }
        }
        Assert.assertTrue("Should be now empty", s_1.isEmpty());
    }
